...
gamesense.DeviceType = {
     ...
     * Initially the Rival 700, Rival 710, Apex 7, Apex 7 TKL, Apex Pro, Apex Pro TKL, Arctis Pro, and GameDAC.
     */
    SCREENED: 'screened',

    // TODO JGN added resolutions
    /**
     * Any connected, supported device that supports notifications on a single OLED or LCD screen.
...
gamesense.ScreenEventHandler = function ScreenEventHandler(deviceType, zone){
    ...
    this.toHandlerData = function toHandlerData() {
        ...
        // TODO JGN fixed!
        if (this.datas) {
            handlerData.datas = this.datas.map(data => {
                if (data.constructor.name === 'RangeScreenData') {
                    return {
                        low: data.low,
                        high: data.high,
                        datas: data.datas.map(frame => toFrameData(frame))
                    };
                } else {
...
// TODO JGN fixed! Typo dans le nom + jsdoc pas à jour
 'use strict';
/**
